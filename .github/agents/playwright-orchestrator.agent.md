...
1. **`playwright-explorer`** — navigates the URL, captures screenshots, enumerates all interactive elements and states, detects login flows, and writes a structured discovery report to `e2e/docs/`.
...
8. **Create `e2e/docs/routes.md`** — immediately after the user confirms the URL queue, create this file at `e2e/docs/routes.md` with a to-do style checklist of every confirmed route. Format each entry as an unchecked GitHub Flavored Markdown task:
...
**Single-URL mode (Mode A):** Invoke one `playwright-explorer` subagent for the single entry. Wait for it to complete and confirm that `e2e/docs/<page-name>/<page-name>-discovery.md` was created. If it fails, surface the error to the user and stop.
...
4. **For each user-selected group:**
   ...
   > "Exploring **`<resource>` group** (`<n>` URLs) — dispatching in batches of up to 3…"
   > b. Split the group's URLs into mini-batches of **at most 3**. Dispatch each mini-batch simultaneously — one `playwright-explorer` subagent per URL in the batch — then wait for all agents in that batch to finish before dispatching the next mini-batch.
   > c. Wait for **all** mini-batches in this group to finish.
   > d. Collect results for the group:
   - **Succeeded**: `e2e/docs/<page-name>/<page-name>-discovery.md` was created.
   - **Failed**: log the error and surface it to the user.
     e. Read every `e2e/docs/<page-name>/<page-name>-test-plan.md` produced for this group. Display them, clearly labelled by page name, then ask:
     ...
     > g. Once approved, **build a generation queue** — list every approved page in order and display it to the user:
     > "**Generation queue for `<resource>` group:**
     >
     > 1. `<page-name-1>` — pending
     > 2. `<page-name-2>` — pending
     >    …"
     >    Then dispatch `playwright-test-generator` **one page at a time**, in queue order. Before starting each page, update its queue entry to `⏳ in progress`. After it finishes, update its entry to `✓ done` or `✗ failed: <reason>`. Do **not** start the next page until the current one has finished. Collect results as you go:
   - **Passed**: all tests green — add to the summary. In batch mode, **update `e2e/docs/routes.md`** — mark each passing route's entry as `- [x]`.
   - **Failed**: log the error and surface it to the user; do not re-attempt automatically. Continue to the next page in the queue.
...
> "Explore `<URL>` and write a discovery report to `e2e/docs/`. Follow your full workflow: verify scaffold, execute any prerequisite steps, navigate to the target page, explore all interactive states **on that page only**, capture screenshots, and write `e2e/docs/<page-name>/<page-name>-discovery.md`.
>
> **Target URL (scope)**: `<URL>` — do NOT follow links or interactions that navigate away from this URL. Only report on this page.
>
> **Silent mode**: Do NOT stream progress updates or intermediate messages to chat. Run silently. When you are finished, reply with a single status line in exactly this format:
> `[Explorer: <page-name>] ✓ Done` — or — `[Explorer: <page-name>] ✗ Failed: <reason>`
>
...
After each mini-batch (or single-URL invocation) completes, print a consolidated status table — one row per explorer — before proceeding:

| Page          | Status                        |
| ------------- | ----------------------------- |
| `<page-name>` | ✓ Done / ✗ Failed: `<reason>` |

For **each** generator invocation, send:

> "Read `e2e/docs/<page-name>/<page-name>-discovery.md` and generate the full test suite. Follow your full workflow: plan the Page Object, generate all files, handle storageState if login was detected, run `cd e2e && npx playwright test --update-snapshots`, and fix any failures until the entire suite is green."
...
In single-URL mode, after the explorer completes, read `e2e/docs/<page-name>/<page-name>-test-plan.md`, display it, and ask:
...
| ----------------------------------------------- | ------------------------- | -------------------------------- |
| `e2e/docs/<page-name>/<page-name>-discovery.md` | playwright-explorer       | Discovery report                 |
| `e2e/docs/<page-name>/<page-name>-*.png`        | playwright-explorer       | Screenshots per state captured   |
| `e2e/pages/<PageName>.ts`                       | playwright-test-generator | Page Object with locators        |
| `e2e/test-data/users.ts`                        | playwright-test-generator | Credentials (if login detected)  |
| `e2e/fixtures/index.ts`                         | playwright-test-generator | Custom fixtures                  |
| `e2e/tests/<feature>.spec.ts`                   | playwright-test-generator | Full spec with test count        |
| `e2e/global-setup.ts`                           | playwright-test-generator | storageState (if login detected) |
| `e2e/docs/routes.md`                            | playwright-orchestrator   | Route coverage checklist (batch) |
...
- DO NOT proceed to Step 4 if the discovery report was not created in Step 2. Discovery reports are located at `e2e/docs/<page-name>/<page-name>-discovery.md`.
...
- In batch mode, URLs within a selected group are dispatched to `playwright-explorer` in mini-batches of **at most 3** simultaneously. Wait for each mini-batch to complete before dispatching the next.
- Each `playwright-explorer` invocation MUST run in **silent mode** — it must not stream intermediate messages to chat. It must reply only with a single `[Explorer: <page-name>] ✓ Done` or `[Explorer: <page-name>] ✗ Failed: <reason>` status line.
- After each mini-batch finishes, the orchestrator MUST print a consolidated status table before continuing.
...
- Before invoking `playwright-test-generator`, ALWAYS build and display a generation queue listing every approved page with a `pending` status.
- `playwright-test-generator` MUST be dispatched **sequentially** — one page at a time, in queue order. NEVER dispatch multiple generator instances simultaneously. Update each queue entry's status (`⏳ in progress` → `✓ done` / `✗ failed`) before moving to the next.
